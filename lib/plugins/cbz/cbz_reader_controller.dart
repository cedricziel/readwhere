...
/// and content streaming for CBZ files. Each "chapter" represents
/// a page in the comic.
class CbzReaderController implements ReaderController {
  ...
  static Future<CbzReaderController> create(String filePath) async {
    _logger.info('Creating CbzReaderController for: $filePath');

    final reader = await cbz.CbzReader.open(filePath);
    final controller = CbzReaderController._(
      reader: reader,
      filePath: filePath,
    );
  ...
  Future<void> _initialize() async {
    ...
    try {
      _logger.info('Initializing CbzReaderController');
  ...
  Future<List<SearchResult>> search(String query) async {
    ...
    _logger.info('Search not supported for CBZ files');
    return [];
  }
  ...
  Future<void> dispose() async {
    ...
    try {
      _logger.info('Disposing CbzReaderController');
...
''';
  ...
  /// Get page metadata.
  cbz.ComicPage? getPageInfo(int index) {
    _ensureInitialized();
  ...
  Uint8List? getPageThumbnail(int index, {cbz.ThumbnailOptions? options}) {
    _ensureInitialized();
